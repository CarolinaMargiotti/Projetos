...
def sum2(nums):
  ...
  return 0 if len(nums)!=1 else nums[0]
...
def date_fashion(eu, par):
  if any(x<=2 for x in (eu,par)):
    return 0
  else:
    if any(x>=8 for x in (eu,par)):
      return 2
  return 1
...
def pego_correndo(speed, is_birthday):
  velMulta=[60,61,80]
  if is_birthday:
    velMulta=[x+5 for x in velMulta]
  if speed<=velMulta[0]:
    return 0
  elif speed>=velMulta[1] and speed<=velMulta[2]:
    return 1
  return 2
