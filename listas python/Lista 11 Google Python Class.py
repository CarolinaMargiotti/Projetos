...
def donuts(n):
    return "Número de donuts: "+(str(n) if n < 10 else "muitos")
...
def pontas(s):
    return s[0:2]+s[-2::1] if len(s) > 2 else ""
...
def palindrome(s):
    return True if s[0:int(len(s)/2)] == s[-int(len(s)/2)::1] else False
...
def busca(frase, palavra):
    count = 0
    for s in range(0, len(frase)):
        if frase[s:s+len(palavra)] == palavra:
            count += 1
        if s == len(frase)-len(palavra):
            break
    return count
